...
public class ComandoRotacionar45 : IExternalCommand
{
    public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
    {
        ...
        try
        {
            Reference refPeca = uidoc.Selection.PickObject(ObjectType.Element, new FiltroConexoes(), "PipeMaster [M]: Selecione a conexão para rotacionar 45° instantaneamente...");
            if (!(doc.GetElement(refPeca) is FamilyInstance { MEPModel: not null } conexao))
            {
                return Result.Cancelled;
            }
            double anguloRadianos = -Math.PI / 4.0;
            ...
            using (Transaction t = new Transaction(doc, "PipeMaster: Girar 45º Rápido"))
            {
                t.Start();
